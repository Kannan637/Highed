...
export const CountryIntakes: React.FC<CountryIntakesProps> = ({ country }) => {
  return (
    <section id="intakes" className="bg-white py-20">
      <Container size="lg">
        <SectionHeading
                ...
                >
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <div className="flex items-center gap-1 text-xs font-semibold text-neutral-400">
                        <Calendar size={14} className="text-[#253A7B]" />
                        <span>Phase {idx + 1}</span>
                      </div>
                    </div>

                    <h3 className="font-heading text-2xl font-bold text-neutral-900 mt-2">
                      {intake.season}
                    </h3>

                    <div className="mt-5 space-y-3 rounded-xl bg-white/80 border border-neutral-200/60 p-4 text-xs sm:text-sm">
                      <div className="flex items-center justify-between">
                        <span className="text-neutral-500 flex items-center gap-1.5">
                          <Clock size={13} />
                          Classes Start:
                        </span>
                        <strong className="text-neutral-900 font-semibold">{intake.months}</strong>
                      </div>
                      <div className="flex items-center justify-between pt-2 border-t border-neutral-100">
                        <span className="text-neutral-500">Apply Before:</span>
                        <strong className="text-[#E93F61] font-semibold">{intake.deadline}</strong>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="mt-6 pt-5 border-t border-neutral-100">
                    <LeadCTAButton
                      ...
                      className={`inline-flex w-full items-center justify-center gap-2 rounded-xl py-2.5 text-xs sm:text-sm font-semibold transition-all cursor-pointer ${
                        isMajor
                          ? "bg-[#253A7B] text-white hover:bg-[#1b2b5c] shadow-xs"
                          : "border border-neutral-300 text-neutral-800 hover:border-[#253A7B] hover:text-[#253A7B] bg-white"
                      }`}
                    >
        ...
        {/* Pro Tip Banner */}
        <div className="mt-8 flex items-start gap-3 rounded-2xl bg-amber-50/80 border border-amber-200/80 p-5 text-sm text-amber-900 shadow-xs">
          <AlertCircle size={20} className="text-amber-600 shrink-0 mt-0.5" />
          <div className="leading-relaxed">
            <strong className="font-semibold text-amber-950">HighEd Application Strategy: </strong>
